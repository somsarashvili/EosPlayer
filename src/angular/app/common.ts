...
export class Common {
  ...
  static calcHealth = function (torrent) {
    const seeds = torrent.seed;
    const peers = torrent.peer;
  ...
  static fileSize = function (num: number) {
    if (isNaN(num)) {
      return;
    }
    ...
    let exponent: number, unit, units, base: number;
    const neg = num < 0;
    ...
    num = +(num / Math.pow(base, exponent)).toFixed(2);
    unit = units[exponent];
